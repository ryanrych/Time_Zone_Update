...
for i in range(len(table)):
    ...
    if page.find("Start DST:")==-1:
        ...
        while page[step]!="<":
            ...
            step+=1
        day=day.split()
        table[i][4]=months.index(day[2])
        #add week
        table[i][6]=day[0][:-1]
        start=page.find("Next Change:")
        start+=168
        step=start
        hour=""
        while page[step]!=",":
            hour+=page[step]
            step+=1
        table[i][7]=hour.split()[3]

        start=page.find("End DST:")+33
        step=start
        day=""
        while page[step]!="<":
            day+=page[step]
